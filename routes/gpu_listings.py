...
# @memory_cache(expiration=120)  # Cache for 1 minute IDK WHY BUT THIS BREAKS EVERYTHING
def get_filtered_gpus():
    try:
        logger.info("Received filter request")
        logger.info("testttttttt")
...
def get_gpu_price_history(gpu_id):
    try:
        ...
        return (
            jsonify({"error": f"Failed to fetch price history for GPU {gpu_id}"}),
            500,
        )
